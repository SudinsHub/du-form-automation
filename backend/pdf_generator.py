...
def generate_cumulative_pdf(db: Session, data): 
    ...
    all_courses = {course.course_code: course for course in db.query(models.Course).all()}
...
def generate_cumulative_pdf(db: Session, data):
    print("[generate_cumulative_pdf] Started PDF generation...")

    try:
        print("[generate_cumulative_pdf] Fetching semester info...")
        semester = db.query(models.ExamSemester).filter(
            models.ExamSemester.id == data.exam_semester_id
        ).first()

        print("[generate_cumulative_pdf] Fetching cumulative report...")
        report_data = crud.get_cumulative_report(db, data.exam_semester_id)

        print("[generate_cumulative_pdf] Fetching all courses...")
        all_courses = {course.course_code: course for course in db.query(models.Course).all()}
        ...
        print("[generate_cumulative_pdf] Fetching chairman info...")
        chairman = db.query(models.Teacher).filter(
            models.Teacher.id == semester.chairman_id
        ).first()

        print("[generate_cumulative_pdf] Rendering HTML...")
        html_content = template.render(
            semester=semester,
            chairman=chairman,
            report_data=report_data,
        ...
        # ----------- PDF GENERATION ------------- #
        print("[generate_cumulative_pdf] Generating PDF...")
        pdf_buffer = BytesIO()
        HTML(string=html_content).write_pdf(pdf_buffer)
        pdf_buffer.seek(0)

        print("[generate_cumulative_pdf] Encoding PDF...")
        pdf_base64 = base64.b64encode(pdf_buffer.read()).decode("utf-8")

        print("[generate_cumulative_pdf] PDF generation successful.")
        ...
        print("[generate_cumulative_pdf] ERROR:", e)
        traceback.print_exc()  # shows details in backend logs
